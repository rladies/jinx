...
jinx is the RLadies+ GitHub organization management bot.
...
| Announcements | `announce_post()`, `announce_post_bluesky()`, `announce_post_mastodon()` |
| Directory | `validate_directory_pr()`, `directory_crop_image()` |
| Blogs | `blog_create_entry()`, `blog_check_links()` |
| Chapters | `chapter_create()`, `chapter_check_health()` |
| Reports | `report_generate()`, `report_publish()` |
| PR review | `review_run()`, `review_check_pr_naming()` |
| Airtable sync | `directory_sync_airtable()`, `gt_sync_airtable()` |
| Website | `website_merge_pending()`, `contributor_greet()` |
| Chapter monitoring | `chapter_monitor_status()`, `prepare_inactivity_emails()` |
| Slack | `slack_invite_batch()`, `slack_subscribe_rss()` |
| GHA dashboard | `gha_generate_dashboard()`, `gha_publish_dashboard()` |
| Contributors | `contributor_list()`, `contributor_update()` |
