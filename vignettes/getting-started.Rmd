...
Issue comment (/jinx invite ...)
  -> issue_comment workflow triggers
    -> installs jinx R package
      -> calls jinx::command_parse() + jinx::command_execute()
...
Commands are parsed by `command_parse()` and dispatched by
`command_execute()`:
...
| Module | Functions |
|--------|-----------|
| Team management | `global_team_invite()`, `global_team_create_offboarding()`, `global_team_finalize_onboarding()` |
| Announcements | `announce_post()`, `post_bluesky()`, `post_mastodon()` |
| Directory | `validate_directory_pr()`, `crop_directory_image()` |
| Blogs | `blog_create_entry()`, `blog_check_links()` |
| Chapters | `create_chapter()`, `chapter_check_health()` |
| Reports | `report_generate()`, `report_publish()` |
| PR review | `review_pr()`, `check_pr_naming()` |
| Airtable sync | `sync_directory_airtable()`, `sync_global_team_airtable()` |
| Website | `website_merge_pending()`, `greet_contributor()` |
| Chapter monitoring | `monitor_chapter_status()`, `prepare_inactivity_emails()` |
| Slack | `slack_invites_send()`, `subscribe_slack_rss()` |
| GHA dashboard | `gha_generate_dashboard()`, `gha_publish_dashboard()` |
| Contributors | `list_contributors()`, `contributors_update()` |
