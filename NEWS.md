...
- The `awesome-rladies-creations` packages feed has 41 entries where
  ...
  rather than strings or `null`. The scheduled indexer crashed mid-run
  on 2026-05-24 with `missing value where TRUE/FALSE needed` when one
  of these reached `nzchar()`. The custom `%or%` operator and the
  downstream `nzchar()` checks now go through a new `is_blank()`
