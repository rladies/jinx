...
create_event_summary <- function(events, period = c("weekly", "monthly")) {
  ...
  event_lines <- vapply(seq_len(nrow(events)), function(i) {
    e <- events[i, ]
    glue::glue("| {e$chapter} | [{e$title}]({e$url}) | {e$date} | {e$rsvp_count} | {e$source} |")
  }, character(1))
...
publish_event_summary <- function(summary,
  ...
  issue <- gh::gh(
    ...
    title = glue::glue("Event Summary - {Sys.Date()}"),
    body = summary,
...
format_event_summary_inline <- function(events, period) {
  event_lines <- vapply(seq_len(nrow(events)), function(i) {
    e <- events[i, ]
    glue::glue("- **{e$chapter}**: [{e$title}]({e$url}) ({e$date}, {e$rsvp_count} RSVPs)")
  }, character(1))
