...
sync_chapter_events <- function(
  ...
  cli::cli_alert_success(
    "Found {nrow(combined)} events across {length(chapters)} chapters"
  )
...
create_event_summary <- function(events, period = c("weekly", "monthly")) {
  ...
  event_lines <- vapply(
    ...
    function(i) {
      e <- events[i, ]
      cli::format_inline(
        "| {e$chapter} | [{e$title}]({e$url}) | {e$date} | {e$rsvp_count} | {e$source} |"
      )
    },
    character(1)
  )
...
format_event_summary_inline <- function(events, period) {
  event_lines <- vapply(
    ...
    function(i) {
      e <- events[i, ]
      cli::format_inline(
        "- **{e$chapter}**: [{e$title}]({e$url}) ({e$date}, {e$rsvp_count} RSVPs)"
      )
    },
    character(1)
  )
