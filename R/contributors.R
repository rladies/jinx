...
#' @export
welcome_contributor <- function(owner, repo, number, author,
                                is_pr = TRUE, org = "rladies") {
  if (is_bot(author)) return(invisible(NULL))
...
#' @export
thank_contributor <- function(owner, repo, pr_number, author,
                              org = "rladies") {
  if (is_bot(author)) return(invisible(NULL))
  ...
  message <- if (first_time) {
    glue::glue(
      ...
      "Thank you for helping make R-Ladies better.\n\n",
      "_Generated by jinx_"
    )
  } else {
    glue::glue(
...
list_contributors <- function(owner, repo, include_bots = FALSE) {
  ...
  df <- data.frame(
    ...
    contributions = vapply(contributors, function(c) c$contributions %||% 0L, integer(1)),
    avatar_url = vapply(contributors, function(c) c$avatar_url %||% "", character(1)),
    profile_url = vapply(contributors, function(c) c$html_url %||% "", character(1)),
...
#' @export
format_contributors <- function(contributors,
                                format = c("table", "grid"),
                                cols = 7) {
...
update_contributors_list <- function(owner, repo,
  ...
  tryCatch(
    ...
    error = function(e) {
      gh::gh(
        ...
        sha = main_ref$object$sha, force = TRUE
      )
    }
  )
  ...
  pr <- gh::gh(
    ...
    body = paste0(
      ...
      "- **Total contributors**: ", nrow(contributors), "\n\n",
      "_Created by jinx_"
    )
  )
...
#' @export
list_org_contributors <- function(org = "rladies",
  ...
  for (r in repos) {
    ...
    for (c in contribs) {
      if (is.null(c$login) || c$type != "User") next
      login <- c$login
      ...
      all_contribs[[login]]$repos <- c(
        all_contribs[[login]]$repos, r$name
      )
  ...
  if (length(all_contribs) == 0) {
    return(data.frame(
      login = character(0), repos = integer(0),
  ...
  df <- data.frame(
    ...
    repos = vapply(all_contribs, function(x) length(unique(x$repos)), integer(1)),
  ...
  row.names(df) <- NULL
  cli::cli_alert_success("Found {nrow(df)} unique contributors across {length(repos)} repos")
  df
}
...
format_contributors_table <- function(contributors) {
  ...
  rows <- vapply(seq_len(nrow(contributors)), function(i) {
    c <- contributors[i, ]
    glue::glue(
...
format_contributors_grid <- function(contributors, cols) {
  ...
  cells <- vapply(seq_len(nrow(contributors)), function(i) {
    c <- contributors[i, ]
    glue::glue(
  ...
  grid_rows <- vapply(seq_len(rows), function(r) {
    start <- (r - 1) * cols + 1
