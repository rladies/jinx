...
#' @export
generate_report <- function(type = c("weekly", "monthly"), org = "rladies",
  ...
  repo_stats <- lapply(repos, function(r) {
    tryCatch(
      ...
      error = function(e) {
        list(repo = r$name, commits = 0, prs_opened = 0,
          prs_merged = 0, issues_opened = 0, issues_closed = 0)
      }
    )
  })
  ...
  report <- list(
    type = type,
    org = org,
    period = list(from = as.character(Sys.Date() - days), to = as.character(Sys.Date())),
    generated_at = Sys.time(),
    repos = repo_stats,
    summary = summarize_stats(repo_stats)
  )

  cli::cli_alert_success("Report generated: {report$summary$total_commits} commits, {report$summary$total_prs} PRs, {report$summary$total_issues} issues")
  invisible(report)
}

collect_repo_stats <- function(org, repo, since) {
  commits <- tryCatch(
    gh::gh("GET /repos/{org}/{repo}/commits",
      org = org, repo = repo, since = since, .limit = Inf),
    error = function(e) list()
  )

  prs <- tryCatch(
    gh::gh("GET /repos/{org}/{repo}/pulls",
      org = org, repo = repo, state = "all", sort = "created",
      direction = "desc", .limit = 50),
    error = function(e) list()
  )

  prs_in_period <- Filter(function(p) p$created_at >= since, prs)
  prs_merged <- Filter(function(p) !is.null(p$merged_at) && p$merged_at >= since, prs)

  issues <- tryCatch(
    gh::gh("GET /repos/{org}/{repo}/issues",
      org = org, repo = repo, state = "all", since = since,
      sort = "created", direction = "desc", .limit = 50),
    error = function(e) list()
  )
  ...
  issues_closed <- Filter(function(i) {
...
summarize_stats <- function(repo_stats) {
  list(
    ...
    total_prs_merged = sum(vapply(repo_stats, function(r) r$prs_merged, integer(1))),
    total_issues = sum(vapply(repo_stats, function(r) r$issues_opened, integer(1))),
    total_issues_closed = sum(vapply(repo_stats, function(r) r$issues_closed, integer(1))),
    active_repos = sum(vapply(repo_stats, function(r) {
...
#' @export
publish_report <- function(report, target_repo = "global-team",
                           org = "rladies") {
