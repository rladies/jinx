...
parse_contributors_command <- function(parts) {
  ...
  list(action = "contributors-list", repo = if (length(parts) >= 2) parts[2] else NULL)
}
...
execute_command <- function(command, context) {
  if (is.null(command)) return(invisible())
  ...
  switch(command$action,
    ...
    "chapter-setup" = {
      ...
      url <- create_chapter_setup(command$city, command$country, organizers = character(0))
      post_reply(owner, repo, context$issue, glue::glue(
    ...
    "report-chapters" = {
      post_reply(owner, repo, context$issue, "Generating chapter health report...")
    ...
    "gha-dashboard" = {
      post_reply(owner, repo, context$issue, "Generating GitHub Actions dashboard...")
    ...
    "contributors-org" = {
      post_reply(owner, repo, context$issue, "Collecting org-wide contributors...")
