...
format_website_analytics <- function(analytics) {
  ...
  overview <- paste(
    ...
    glue::glue("| Avg. visit duration | {format_duration(agg$visit_duration$value)} |"),
    sep = "\n"
  )
...
format_website_slack <- function(report_data, issue_url) {
  ...
  lines <- c(
    glue::glue(":globe_with_meridians: *Website Analytics ({site}) - {Sys.Date()}*"),
    glue::glue("Period: {period}"),
    "",
    glue::glue(":busts_in_silhouette: Visitors: *{agg$visitors$value}* | :page_facing_up: Pageviews: *{agg$pageviews$value}*"),
    glue::glue(":leftwards_arrow_with_hook: Bounce rate: {agg$bounce_rate$value}% | :clock1: Avg. duration: {format_duration(agg$visit_duration$value)}")
  )
