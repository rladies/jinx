...
compute_activity_trends <- function(activity_data) {
  ...
  monthly$change <- c(NA_real_, diff(monthly$commits) / head(monthly$commits, -1) * 100)
...
format_analytics_markdown <- function(trends, contributor_growth) {
  template_path <- system.file(
    "templates", "analytics-dashboard.md",
    package = "jinx"
  )
  ...
  chapter_lines <- if (nrow(trends) > 0) {
    paste(
      ...
      "|-------|---------|--------|-------|",
      paste(vapply(seq_len(nrow(trends)), function(i) {
  ...
  growth_lines <- if (nrow(contributor_growth) > 0) {
    paste(
      "| Month | New | Total |",
      "|-------|-----|-------|",
      paste(vapply(seq_len(nrow(contributor_growth)), function(i) {
        cli::format_inline(
  ...
  if (nzchar(template_path)) {
    render_template(template_path, list(
      PERIOD = cli::format_inline("{trends$month[1]} to {trends$month[nrow(trends)]}"),
...
summarize_top_chapters <- function(trends) {
  if (nrow(trends) == 0) return("No data.")
